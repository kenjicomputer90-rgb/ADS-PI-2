...
export async function criarLocacao(data: {
  ...
  if (statusAtual.id_status !== STATUS_DISPONIVEL) {
    throw new Error("Peça não está disponível para locação")
  }
...
export async function cancelarLocacao(idLocacao: number) {
  ...
  if (locacao.status === "CANCELADA") {
    throw new Error("Locação já está cancelada")
  }

  if (locacao.status === "DEVOLVIDA") {
    throw new Error("Locação devolvida não pode ser cancelada")
  }
  ...
  return await prisma.locacao.update({
    where: {
      id_locacao: idLocacao,
    },
    data: {
...
export async function entregarLocacao(idLocacao: number) {
  ...
  if (locacao.status !== "RESERVADA") {
    throw new Error("Só é possível entregar uma locação reservada")
  }
...
export async function devolverLocacao(idLocacao: number) {
  ...
  if (locacao.status !== "ENTREGUE") {
    throw new Error("Só é possível devolver uma locação entregue")
  }
...
export async function trocarPeca(idLocacao: number, novaPecaId: number) {
  ...
  if (locacao.status !== "RESERVADA") {
    throw new Error("Só é possível trocar peça de uma locação reservada")
  }

  if (locacao.item_locacao.length === 0) {
    throw new Error("Locação não possui peça vinculada")
  }

  const statusNovaPeca = await buscarStatusAtualPeca(novaPecaId)

  if (!statusNovaPeca) {
    throw new Error("Nova peça não possui status cadastrado")
  }

  if (statusNovaPeca.id_status !== STATUS_DISPONIVEL) {
    throw new Error("Nova peça não está disponível")
  }
