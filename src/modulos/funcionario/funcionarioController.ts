...
// 🔵 LINHA MODIFICADA: Adicionado 'listFuncionario' no final do import abaixo para puxar do Service
import { returnFuncionario, changeFuncionario, removeFuncionario, addFuncionario, listFuncionario } from "./funcionarioService.js";

// 🟢 BLOCO NOVO: Nova rota que lê os funcionários do banco usando o Service do grupo
export const listFuncionarioController = async (req: Request, res: Response) => {
  try {
    const funcionarios = await listFuncionario();
...
export const addFuncionarioController = async (req: Request, res: Response) => {
  try {
    ...
    // 🔵 LINHA MODIFICADA: Removido o 'Number()' em volta de id_usuario para aceitar o texto "Manoel"
    const resultado = await addFuncionario(
      id_usuario, nome, cpf, rg, telefone, ctps,
